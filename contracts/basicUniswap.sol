...
contract BasicUniswap is ERC20 {
    error BasicUniswap__DeadlineHasPassed(uint64 deadline);
    error BasicUniswap__MaxPoolTokenDepositTooHigh(
        uint256 maximumPoolTokensToDeposit,
        uint256 poolTokensToDeposit
    );
    error BasicUniswap__MinLiquidityTokensToMintTooLow(
        uint256 minimumLiquidityTokensToMint,
        uint256 liquidityTokensToMint
    );
    error BasicUniswap__WethDepositAmountTooLow(
        uint256 minimumWethDeposit,
        uint256 wethToDeposit
    );
    error BasicUniswap__InvalidToken();
    error BasicUniswap__OutputTooLow(uint256 actual, uint256 min);
    error BasicUniswap__MustBeMoreThanZero();
        ...
        returns (uint256 liquidityTokensToMint)
    {
        if (wethToDeposit < MINIMUM_WETH_LIQUIDITY) {
            revert BasicUniswap__WethDepositAmountTooLow(
        ...
        if (totalLiquidityTokenSupply() > 0) {
            ...
            if (maximumPoolTokensToDeposit < poolTokensToDeposit) {
                revert BasicUniswap__MaxPoolTokenDepositTooHigh(
            ...
            if (liquidityTokensToMint < minimumLiquidityTokensToMint) {
                revert BasicUniswap__MinLiquidityTokensToMintTooLow(
        ...
        revertIfZero(minPoolTokensToWithdraw)
    {
        ...
        if (wethToWithdraw < minWethToWithdraw) {
            revert BasicUniswap__OutputTooLow(
                wethToWithdraw,
                minWethToWithdraw
            );
        }
     ...
     * @dev Every 10 swaps, we give the caller an extra token as an extra incentive to keep trading on BASIC-Uniswap.
     * @param inputToken ERC20 token to pull from caller
